...
[출력 요구사항]
- 정확히 3분(180초) 분량의 대본 (약 1350-1800자, 최소 1500자 이상)
- 오프닝 훅: 시청자의 관심을 끄는 강렬한 질문이나 문장 (약 45-60자)
- 본문: 사실 → 원인 → 구조 → 맥락 순서로 매우 상세하게 전개 (약 1200-1500자)
  * 사실: 구체적인 통계, 데이터, 사례를 매우 충분히 제시하고 여러 각도에서 설명
  * 원인: 왜 이런 일이 발생했는지 매우 깊이 있게 다각도로 분석
  * 구조: 문제의 근본적인 구조와 메커니즘을 상세하게 설명하고 관련 요소들을 연결
  * 맥락: 더 넓은 사회적, 경제적, 문화적 맥락을 깊이 있게 연결하고 역사적 배경도 포함
- 인사이트 요약: 핵심 내용을 3-4문장으로 상세하게 정리 (약 75-90자)
- 마무리: 반복 시청을 유도하는 멘트 (약 45-60자)
- 자연스러운 구어체로 작성하되, 내용이 매우 풍부하고 상세해야 함
- 각 섹션에서 예시, 비유, 구체적 사례를 충분히 포함할 것
- 시간 정보나 시간 표시는 포함하지 말 것
- 대본만 작성하고 불필요한 설명은 제외
- 반드시 3분 분량에 맞는 충분한 길이로 작성할 것 (최소 1500자 이상 필수)`;
