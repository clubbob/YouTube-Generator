...
채널 말투: 차분하고 분석적인 톤을 유지하되, 딱딱한 보고서 말투는 절대 금지. 시청자의 감정을 붙잡는 **따뜻한 공감**과 **생생한 장면감**을 반드시 함께 넣는다. 감정적 선동, 판단 강요, 훈계 어조는 사용하지 않는다. '설명해주는 사람'의 시점으로 말하되, **스토리텔링 방식**으로 전달한다.

**작성 원칙 (매우 중요 - 반드시 준수):**
- **밋밋한 설명 절대 금지**: "~입니다", "~합니다" 같은 딱딱한 설명문은 피한다
- **구체적 사례와 숫자 필수**: 추상적 설명보다 구체적 사례와 숫자를 반드시 포함
- **장면감 있는 묘사**: 설명보다는 상황과 장면을 묘사하여 시청자가 상상할 수 있게
- **스토리텔링 우선**: 정보 나열보다는 이야기로 전달
- **임팩트 있는 문장**: 평범한 설명보다는 시청자의 감정을 건드리는 문장 사용
...
2. 핵심 설명 (본문)
- **절대 금지: 밋밋한 설명, 정보 나열, 딱딱한 보고서 톤**
- **필수: 구체적 사례, 숫자, 장면감, 스토리텔링 방식**
- 전개 순서: 사실 → 원인 → 구조 → 맥락
- 설명형 나열이 아니라 '질문 → 답변' 흐름으로 작성
- 한 문장은 최대 2줄을 넘지 않게 작성
- **중요: 본문은 반드시 1400자 이상이어야 합니다. 3분 분량을 채우기 위해 충분한 내용이 필요합니다.**
- **분량 확보 및 임팩트 강화를 위해 (반드시 포함):**
  * **구체적 숫자/통계 필수**: "500만원", "3년", "20대", "30% 증가" 같은 구체적 숫자를 반드시 포함
  * **실제 사례 구체적 제시**: "2023년 한국은행이 금리를 인상했을 때..." 같은 구체적 시기와 상황 포함
  * **장면감 있는 묘사**: "A씨는 아침 7시에 일어나..." 같은 구체적 시간과 행동 묘사
  * **비교 설명**: "과거에는 ~였지만, 지금은 ~" 같은 비교를 통해 변화를 명확히
  * **배경 지식이나 역사적 맥락**: 단순 나열이 아닌 스토리로 연결
  * **다양한 관점**: 한 가지 설명이 아닌 여러 측면을 다룸
  * **스토리텔링 방식**: 설명보다는 사례와 이야기로 전달
  * **단순 요약 금지**: 충분한 설명과 예시를 포함하여 임팩트 있게 작성

────────────────

(1) 감정 장면 삽입 규칙 (매우 중요 - 임팩트의 핵심)
- 본문 초반(첫 1~2문단)에 **사람이 보이는 장면 1개**를 반드시 넣는다. (4~5문장, 일상적 상황/짧은 사례/대화 한 줄. 실명·과장 금지)
- **구체적 묘사 필수**: 시간, 장소, 행동을 구체적으로 묘사 (예: "아침 7시, 지하철역 앞 시간제 근무 직업소개소 앞에서...")
- 이 인물은:
  - 판단하거나 결론을 내리지 않는다
  - 대신 망설이거나, 헷갈리거나, 불편해한다, 고민한다
  - 일상적 상황 / 짧은 내적 독백 1줄 허용
  - **구체적 행동과 감정을 함께 묘사** (예: "한숨을 쉬며", "고개를 갸우뚱하며")
- 목적: 시청자를 '관찰자'가 아니라 '당사자'로 만든다
- **밋밋한 설명 대신 생생한 장면으로 시작**
...
- **밋밋한 설명 전환 금지**: "~입니다", "~합니다" 같은 딱딱한 문구 대신 스토리텔링 방식으로 자연스럽게 연결
...
────────────────
[출력 요구사항]
- 유튜브 영상 제목 (매우 중요 - 클릭률을 결정하는 핵심):
  * 30~40자 이내
  * **절대 금지: 정보 나열, 기사 헤드라인 톤, 평범한 설명, 단순 질문형**
  * 제목이 '뉴스'처럼 보이면 구독이 멈춘다. **기사 냄새(헤드라인 톤)** 가 나면 실패다.
  * **평범한 질문형 제목도 절대 금지**: "~이 필요한 이유는 무엇일까", "~은 어떻게 될까" 같은 단순 질문형은 클릭률이 매우 낮음
  * **필수 요소: 호기심 유발, 반전/충격 요소, 구체적 사실 포함, 예상 밖의 관점**
  * 제목 작성 전략 (반드시 다음 중 하나 이상 포함):
    1. **반전 구조 필수**: "~인 줄 알았는데", "~라고 생각했지만", "실제로는", "숨겨진 진실", "~이 아니라 ~였다"
    2. **구체적 숫자/사실 포함**: 주제와 관련된 구체적 숫자, 사례, 현상을 제목에 반드시 포함 (예: "500만원", "3년", "20대")
    3. **호기심 유발 키워드**: "진짜", "비밀", "숨겨진", "실제로는", "의외로", "충격", "놀라운"
    4. **감정적 반응 유도**: "충격", "놀라운", "의외의", "숨겨진", "진짜" 등으로 시청자의 감정을 자극
    5. **예상 밖의 관점**: 일반적인 설명이 아닌, 대부분이 모르는 사실이나 반전된 관점 제시
  * **제목 끝에 항상 마침표(.)를 추가할 것**
  * **나쁜 예**: 
    - "환율은 어떻게 결정될까" (평범함, 구체성 부족)
    - "청년들에게 주택청약통장이 필요한 이유는 무엇일까" (단순 질문형, 평범함)
    - "주택청약통장이란 무엇인가" (정보 나열, 평범함)
  * **좋은 예**: 
    - "환율이 오르는 진짜 이유는 달러가 아니라 이 한 가지였다" (반전, 호기심, 구체성)
    - "주택청약통장, 대부분이 놓치는 이 한 가지 때문에 실패한다" (반전, 호기심)
    - "20대가 주택청약통장을 시작하지 말아야 하는 이유가 있다" (반전, 충격)
    - "주택청약통장 500만원 모으는 사람들이 절대 하지 않는 실수 하나" (구체적 숫자, 반전)
- 전체 대본 분량:
...
**날짜 언급 규칙 (매우 중요 - 반드시 준수):**
- 날짜를 언급할 때는 반드시 구체적으로 표기해야 함
- **절대 금지**: "24일", "15일"처럼 날짜만 언급 (시청자가 언제인지 모름)
- **필수**: "1월 24일", "2026년 1월 24일"처럼 월과 일을 함께 표기
- 예시:
  * 나쁜 예: "24일 발표된 조사에 따르면"
  * 좋은 예: "1월 24일 발표된 조사에 따르면" 또는 "2026년 1월 24일 발표된 조사에 따르면"
- 뉴스 기사나 주제에 나온 날짜 정보를 그대로 사용하되, "일"만 있는 경우에는 월과 년도도 함께 표기
- 인사이트 요약:
  * 3~4문장
  * 약 75~90자
- 마무리:
...
5. **날짜 언급 규칙 (매우 중요):**
   ...
   - 예시:
     * 나쁜 예: "24일 발표된 조사에 따르면"
     * 좋은 예: "1월 24일 발표된 조사에 따르면" 또는 "2026년 1월 24일 발표된 조사에 따르면"
   - 뉴스 기사에 나온 날짜 정보를 그대로 사용하되, "일"만 있는 경우에는 월과 년도도 함께 표기

6. **검증 방법:**
   - 대본을 작성한 후, 뉴스 기사에 나온 모든 주요 정보(숫자, 이름, 제품명, 날짜 등)가 대본에 포함되었는지 확인
   - 날짜가 "일"만 언급되어 있는지 확인하고, 그렇다면 월과 년도도 함께 표기했는지 확인
   - 빠진 정보가 있다면 반드시 추가
...
유튜브 영상 제목 (매우 중요 - 클릭률을 결정하는 핵심):
- 30~40자 이내
- **절대 금지: 정보 나열, 기사 헤드라인 톤, 평범한 설명**
- **필수 요소: 호기심 유발, 반전/충격 요소, 구체적 사실 포함**
- 제목 작성 전략:
  1. **반전 구조 사용**: "~인 줄 알았는데", "~라고 생각했지만", "실제로는", "숨겨진 진실"
  2. **구체적 숫자/사실 포함**: 뉴스 기사에 나온 구체적 숫자, 기업명, 제품명을 제목에 반드시 포함
  3. **호기심 유발 키워드**: "왜", "어떻게", "진실", "비밀", "숨겨진", "실제로는", "의외로"
  4. **감정적 반응 유도**: "충격", "놀라운", "의외의", "숨겨진" 등으로 시청자의 감정을 자극
  5. **질문형 구조**: 물음표(?)는 사용하지 않지만, 문장 자체를 의문형으로 끝냄 (예: ~일까, ~왜일까, ~어쩌다 이런 일이 반복될까)
- **제목 끝에 항상 마침표(.)를 추가할 것**
- **나쁜 예**: "환율 상승의 원인과 전망" (정보 나열, 평범함)
- **좋은 예**: "환율이 1500원을 넘긴 진짜 이유는 달러가 아니라 이것이었다" (반전, 구체적 숫자, 호기심)
    ...
    try {
      ...
      if (knowledgeResponse.ok) {
        ...
        if (knowledgeData.success && knowledgeData.template) {
          ...
          // 최신 템플릿에 필수 섹션이 있는지 확인 (프롬프트 개선 사항 반영)
          const hasRequiredSections = latestKnowledgeTemplate.includes("최소 1700자 이상 필수") && 
                                     latestKnowledgeTemplate.includes("분량 검증") &&
                                     latestKnowledgeTemplate.includes("밋밋한 설명 절대 금지") &&
                                     latestKnowledgeTemplate.includes("구체적 사례와 숫자 필수") &&
                                     latestKnowledgeTemplate.includes("스토리텔링 우선") &&
                                     latestKnowledgeTemplate.includes("감정 장면 삽입 규칙") &&
                                     latestKnowledgeTemplate.includes("반전 구조 필수") &&
                                     latestKnowledgeTemplate.includes("단순 질문형 제목도 절대 금지");
          
          // 버전이 날짜 형식이 아니면 업데이트 필요 (예: "1.0" -> "1.0.YYYYMMDD")
          const versionNeedsUpdate = !dbVersion.includes(".") || dbVersion.split(".").length < 3 || 
                                     dbVersion === "1.0" || 
                                     dbVersion === KNOWLEDGE_SCRIPT_PROMPT_VERSION ||
                                     !dbVersion.match(/\d{8}$/); // 날짜 형식이 아니면 업데이트
          
          // 내용이 변경되었거나 필수 섹션이 없거나 버전 형식이 맞지 않으면 업데이트
