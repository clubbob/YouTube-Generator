...
다음 뉴스 기사를 바탕으로
"시청자의 관심을 끌고 끝까지 보게 만드는 3분(180초) 유튜브 숏폼 영상 대본과 제목"을 작성해라.
...
[출력 요구사항]
- 유튜브 영상 제목: 클릭을 유도하고 대본의 핵심을 담은 제목 1개 추천 (30-40자 이내)
- 3분(180초) 분량의 대본 (약 1500자)
- 오프닝 훅: 시청자의 관심을 끄는 강렬한 질문이나 문장 (약 45-60자)
  * 설명이 아니라 "이 영상을 끝까지 보면 무엇을 알게 되는지" 또는 "일반적인 생각과 다른 관점"을 반드시 포함할 것
  * 시청자가 멈춰서 보게 만드는 호기심 유발 문장이어야 함
- 본문: 사실 → 원인 → 구조 → 맥락 순서로 전개하되, 리듬감 있게 작성 (약 1300자)
  * 사실: 구체적인 통계, 데이터, 사례를 제시하되 과도한 설명은 피하고 핵심만 전달
  * 원인: 왜 이런 일이 발생했는지 분석하되, 설명형 논문이 아닌 호흡감 있는 문체로 작성
  * 구조: 문제의 근본적인 구조와 메커니즘을 설명하고 관련 요소들을 연결
  * 맥락: 더 넓은 사회적, 경제적, 문화적 맥락을 연결하되 역사적 배경은 간결하게
  * 본문 중간에 반드시 "그런데 여기서 대부분이 놓치는 지점이 있습니다" 또는 "이 부분이 앞으로 더 중요해질 수 있습니다" 같은 긴장 유지 문장을 포함할 것
- 인사이트 요약: 핵심 내용을 3-4문장으로 상세하게 정리 (약 75-90자)
- 마무리: 다음 영상과 직접 연결되는 질문 형태로 작성하고 구독/좋아요 추천 문구 포함 (약 60-80자)
  * "이 이슈의 다음 단계 또는 반대 관점"을 질문 형태로 제시할 것
  * 예: "그런데 이와 반대되는 관점은 무엇일까요? 다음 영상에서 다뤄보겠습니다."
  * 구독과 좋아요 추천 문구 (예: "구독과 좋아요를 눌러주세요. 최근 뉴스와 정보를 제공해 드립니다.")
- 자연스러운 구어체로 작성하되, 설명 위주가 아닌 리듬감과 긴장감을 유지할 것
- 각 섹션에서 예시, 비유, 구체적 사례를 포함하되 과도하지 않게
- 시간 정보나 시간 표시는 포함하지 말 것
- 출력 형식: 먼저 "[영상 제목]" 다음 줄에 제목을 작성하고, 그 다음 "[대본]" 다음 줄에 대본을 작성할 것`;
